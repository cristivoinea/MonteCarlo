...
for r in radii:
    if SWAP_term == 'p':
        RunPSWAP(Ne=Ne, Ns=Ns, t=t, M=M, M0=M0, step_size=step,
                 region_geometry='circle', boundary=r,
                 state=state)
    if SWAP_term == 'mod':
        RunModSWAP(Ne=Ne, Ns=Ns, t=t, M=M, M0=M0, step_size=step,
                   region_geometry='circle', boundary=r,
                   state=state)
    if SWAP_term == 'sign':
        RunSignSWAP(Ne=Ne, Ns=Ns, t=t, M=M, M0=M0, step_size=step,
                    region_geometry='circle', boundary=r,
                    state=state)
