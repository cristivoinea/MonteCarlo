...
class MonteCarloSphereFreeFermions(MonteCarloSphere):
            ...
            for j in range(i + 1, self.N):
                if Ms[i] == Ms[j]:
                    overlap_matrix[i, j] = (
                        norms[i]
                        * norms[j]
                        * LegendreOffDiagIntegral(
                            x, legendre, int(Ls[i]), int(Ls[j]), int(Ms[i])
                        )
                    )
    ...
    def InitialWavefn(self):
        ...
        for copy in range(nbr_copies):
            self.InitialSlater(
                self.coords[copy * self.N : (copy + 1) * self.N], self.slater[..., copy]
            )  # , self.slater_inverse[..., copy])
            self.slogdet[:, copy] = np.linalg.slogdet(self.slater[..., copy])
    ...
    def InitialWavefnSwap(self):
        ...
        for copy in range(2):
            self.InitialSlater(
                coords_swap[copy * self.N : (copy + 1) * self.N],
                self.slater[..., 2 + copy],
            )  # , self.slater_inverse[..., 2+copy])
            self.slogdet[:, 2 + copy] = np.linalg.slogdet(self.slater[..., 2 + copy])
    ...
    def TmpWavefn(self):
        for copy in range(self.moved_particles.size):
            ...
            self.slogdet_tmp[:, copy] = np.linalg.slogdet(self.slater_tmp[..., copy])
    ...
    def TmpWavefnSwap(self):
        ...
        for copy in range(2):
            ...
            self.slater_tmp[:, moved_in_swap % self.N, 2 + swap_copy] = sph_harm(
                self.Ls[:, 1] - self.Ls[:, 0] - self.S_eff / 2,
                self.Ls[:, 0] + self.S_eff / 2,
                coords_swap_tmp[moved_in_swap, 1],
                coords_swap_tmp[moved_in_swap, 0],
            )
    ...
    def StepAmplitude(self) -> np.complex128:
        ...
        for copy in range(nbr_copies):
            step_amplitude *= (
                self.slogdet_tmp[0, copy] / self.slogdet[0, copy]
            ) * np.exp(self.slogdet_tmp[1, copy] - self.slogdet[1, copy])
    ...
    def StepAmplitudeTwoCopiesSwap(self) -> np.complex128:
        ...
        for copy in range(2):
            step_amplitude *= (
                self.slogdet_tmp[0, 2 + copy] / self.slogdet[0, 2 + copy]
            ) * np.exp(self.slogdet_tmp[1, 2 + copy] - self.slogdet[1, 2 + copy])
        return step_amplitude
    ...
    def InitialSign(self):
        step_amplitude = 1
        for copy in range(2):
            step_amplitude *= np.conj(self.slogdet[0, copy + 2]) * self.slogdet[0, copy]
    ...
    def __init__(
        ...
        self.slater = np.zeros((N, N, 4 ** (nbr_copies - 1)), dtype=np.complex128)
