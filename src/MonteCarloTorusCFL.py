...
def njit_UpdateJastrows(t: np.complex128, Lx: np.float64,
                        ...
                        moved_particle: np.uint16, projected_flag: np.bool_):
    Ne = coords_tmp.size
    if projected_flag:
        ...
        if pfaffian_flag:
            for i in range(Ne):
                if i != moved_particle:
                    pfaffian_jastrows_tmp[i, moved_particle] = ThetaFunction(
                        (coords_tmp[i] - coords_tmp[moved_particle])/Lx, t, 0, 0)

                    pfaffian_matrix_tmp[i, moved_particle] = (pfaffian_jastrows_tmp[i, moved_particle] /
                                                              jastrows_tmp[i, moved_particle])
                    pfaffian_matrix_tmp[moved_particle, i] = - \
                        pfaffian_matrix_tmp[i, moved_particle]"""
...
class MonteCarloTorusCFL (MonteCarloTorusBase):
    ...
    projected: np.bool_
    pfaffian_flag: np.bool_
    pfaffian_jastrows: np.array
    pfaffian_jastrows_tmp: np.array
    pfaffian_matrix: np.array
    pfaffian_matrix_tmp: np.array
    ...
    def InitialJastrows(self, coords, jastrows):
        if self.projected:
                    ...
                    for j in range(i+1, self.Ne):
                        for l in range(self.JK_coeffs_unique.shape[1]):
                            jastrows[i, j, k, l] = ThetaFunction(
                                (coords[i] - coords[j] +
                                 self.JK_coeffs_unique[0, l]*1j*self.Ks[k])/self.Lx,
                                self.t, 1/2, 1/2)
            ...
            for i in range(self.Ne):
                for j in range(i+1, self.Ne):
                    jastrows[i, j, 0, 0] = ThetaFunction(
                        (coords[i] - coords[j])/self.Lx, self.t, 1/2, 1/2)
                    jastrows[j, i, 0, 0] = - jastrows[i, j, 0, 0]

                if self.pfaffian_flag:
                    self.pfaffian_jastrows[i, j] = ThetaFunction(
                        (coords[i] - coords[j])/self.Lx, self.t, 0, 0)
                ...
                for m in range(self.Ne):
                    ...
                    for l in range(self.JK_coeffs_unique.shape[1]):
                        JK_matrix[n, m] *= np.prod(
                            np.power(jastrows[m, :, n, l], self.JK_coeffs_unique[1, l]))
        else:
    ...
    def TmpWavefn(self):
        ...
        elif self.coords.size == self.Ne:
            njit_UpdateJastrows(self.t, self.Lx, self.coords_tmp,
                                self.Ks, self.jastrows, self.jastrows_tmp,
                                self.JK_coeffs_unique, self.JK_matrix_tmp,
                                self.moved_particles[0], self.projected)
    ...
    def StepAmplitude(self) -> np.complex128:
        ...
        (reduced_CM, expo_CM) = self.ReduceThetaFunctionCM(np.sum(
            self.coords + 1j*self.Ks*self.projected))
        (reduced_CM_tmp, expo_CM_tmp) = self.ReduceThetaFunctionCM(np.sum(
            self.coords_tmp + 1j*self.Ks*self.projected))

        moved_coord = self.coords[self.moved_particles[0]]
        moved_coord_tmp = self.coords_tmp[self.moved_particles[0]]
        expo_nonholomorphic = 1j*(moved_coord_tmp*np.imag(moved_coord_tmp) -
                                  moved_coord*np.imag(moved_coord))/2

        step_amplitude *= (reduced_CM_tmp * self.JK_slogdet_tmp[0] /
                           (reduced_CM * self.JK_slogdet[0]))
        step_exponent = expo_CM_tmp - expo_CM + expo_nonholomorphic + \
            self.JK_slogdet_tmp[1] - self.JK_slogdet[1]
        ...
        if self.projected:
            if self.Ns//self.Ne % 2 == 0:
                ...
                step_amplitude *= (np.prod(np.exp(step_exponent/self.Ne) *
                                           self.jastrows[self.moved_particles[0], :, 0, 0] /
                                           self.jastrows_tmp[self.moved_particles[0], :, 0, 0]))
            elif self.Ns//self.Ne == 3:
                step_amplitude *= (np.prod(np.exp(step_exponent/self.Ne) *
                                           self.jastrows_tmp[self.moved_particles[0], :, 0, 0] /
                                           self.jastrows[self.moved_particles[0], :, 0, 0]))
            ...
            step_amplitude *= (np.prod(np.exp(step_exponent/self.Ne) *
                                       np.power(self.jastrows_tmp[self.moved_particles[0], :, 0, 0] /
                                                self.jastrows[self.moved_particles[0], :, 0, 0], self.Ns/self.Ne)))
    ...
    def __init__(self, Ne, Ns, t, nbr_iter, nbr_nonthermal, region_geometry,
                 region_size, step_size, JK_coeffs='0', pfaffian_flag=False,
        ...
        coeffs, counts = np.unique(
            JK_coeffs_parsed, return_counts=True)
        self.JK_coeffs_unique = np.vstack((coeffs, counts))
        ...
        self.pfaffian_flag = pfaffian_flag

        if nbr_copies == 1:
            self.jastrows = np.ones(
                (Ne, Ne, Ne**self.projected, self.JK_coeffs_unique.shape[1]), dtype=np.complex128)
            self.JK_matrix = np.zeros((Ne, Ne), dtype=np.complex128)
            self.JK_slogdet = np.zeros(2, dtype=np.complex128)

            if self.pfaffian_flag:
                self.pfaffian_jastrows = np.zeros(
                    (Ne, Ne), dtype=np.complex128)
                self.pfaffian_jastrows_tmp = np.copy(self.pfaffian_jastrows)

        elif nbr_copies == 2:
            self.jastrows = np.ones(
                (2*Ne, 2*Ne, Ne**self.projected, self.JK_coeffs_unique.shape[1]), dtype=np.complex128)
            self.JK_matrix = np.zeros((Ne, Ne, 4), dtype=np.complex128)
            self.JK_slogdet = np.zeros((2, 4), dtype=np.complex128)

            if self.pfaffian_flag:
                self.pfaffian_jastrows = np.zeros(
                    (2*Ne, 2*Ne), dtype=np.complex128)
                self.pfaffian_jastrows_tmp = np.copy(self.pfaffian_jastrows)
