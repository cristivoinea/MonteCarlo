...
class MonteCarloTorus(MonteCarloBase):
    ...
    def InsideRegion(self, coords):
        ...
        if self.region_geometry == "strip":
            ...
            inside_A = (np.abs(x) < self.boundary) & (np.abs(y) < self.boundary)
    ...
    def ReduceNonholomorphic(
        ...
        expo_nonholomorphic = (
            np.sum(np.power(coords, 2) - np.power(np.abs(coords), 2)) / 4
        )
        """
    ...
    def ReduceThetaFunctionCM(
        self, zCM: np.complex128
