import argparse
import numpy as np
from MonteCarloSWAPTorus import RunModSWAP
...
parser.add_argument("--step-const", action="store", required=True,
                    help="step constant (step / r)")
...
Ns = np.uint8(args["Ns"])
M = np.uint32(args["M"])
step_const = np.float64(args["step_const"])
...
for t in ts:
    step_size = step_const*np.sqrt(np.imag(t))
    result, acceptance = RunModSWAP(N=Ne, Ns=Ns, t=t, M=M, step_size=step_size,
                                    boundary_dimensionless=boundary)
    final = np.hstack((np.array([acceptance]), result))
    np.save(
        f"mod_swap_Ne_{Ne}_Ns_{Ns}_t_{np.imag(t):.2f}_step_{step_size:.3f}.npy", final)
